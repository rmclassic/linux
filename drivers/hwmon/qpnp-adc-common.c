...
int32_t qpnp_adc_scale_millidegc_pmic_voltage_thr(
		...
		uint32_t *low_threshold, uint64_t *high_threshold)
{
		...
		uint32_t *low_threshold, uint64_t *high_threshold)
{
	...
	uint64_t low_threshold64 = *low_threshold; 

	qpnp_get_vadc_gain_and_offset(&usb_param, CALIB_RATIOMETRIC);
	
	*low_threshold = param->low_thr * usb_param.dy;
	do_div(low_threshold64, usb_param.adc_vref);
	*low_threshold = low_threshold64;
...
static void qpnp_adcmap_remove(struct spmi_device *pdev)
{
	if (adcmap_use_dt) {
		kfree(adcmap_btm_threshold);
		kfree(adcmap_qrd_btm_threshold);
