...
static int32_t qpnp_iadc_read_reg(uint32_t reg, u8 *data)
{
	...
	rc = spmi_ext_register_readl_legacy(iadc->adc->spmi->ctrl, iadc->adc->slave,
...
static int32_t qpnp_iadc_write_reg(uint32_t reg, u8 data)
{
	...
	buf = &data;
	rc = spmi_ext_register_writel_legacy(iadc->adc->spmi->ctrl, iadc->adc->slave,
...
static void qpnp_iadc_remove(struct spmi_device *spmi)
{
